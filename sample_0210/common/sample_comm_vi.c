...
static combo_dev_attr_t g_mipi_4lane_chn0_sensor_hw_12bit_8m_nowdr_attr = {
    .devno = 0,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate  = MIPI_DATA_RATE_X1,
    .img_rect   = {0, 0, 1024, 768},
    .mipi_attr = {
    ...
    hi_u32 *ob_height)
{
    switch (sns_type) {
        ...
        case HW_DC:
           // *ob_height = OB_HEIGHT_END;
            (hi_void)memcpy_s(combo_attr, sizeof(combo_dev_attr_t),
                &g_mipi_4lane_chn0_sensor_hw_12bit_8m_nowdr_attr, sizeof(combo_dev_attr_t));
            break;
...
static hi_vi_dev_attr hw_t601_dev_attr = {
    .intf_mode = OT_VI_INTF_MODE_BT601,

    /* Invalid argument */
    .work_mode = HI_VI_WORK_MODE_MULTIPLEX_1,

    /* mask component */
    .component_mask = {0x00ff0000,0x00000000}, // 
...
static hi_vi_dev_attr hw_dc_dev_attr = {
    ...
    .sync_cfg = {
        ...
        .hsync_neg       = OT_VI_HSYNC_NEG_HIGH,
        .vsync_valid     = OT_VI_VSYNC_NORM_PULSE,
        .vsync_valid_neg = OT_VI_VSYNC_NORM_PULSE,
        .timing_blank    = {
            /* hsync_hfb      hsync_act     hsync_hhb */
            64,                1024,            250,
            /* vsync0_vhb     vsync0_act    vsync0_hhb */
...
static hi_void sample_comm_vi_get_dev_attr_by_intf_mode(hi_vi_intf_mode intf_mode, hi_vi_dev_attr *dev_attr)
{
    switch (intf_mode) {
        ...
        case HI_VI_INTF_MODE_BT1120:
            ...
            break;
        case HI_VI_INTF_MODE_BT601:
             (hi_void)memcpy_s(dev_attr, sizeof(hi_vi_dev_attr), &hw_t601_dev_attr, sizeof(hi_vi_dev_attr));
             break;
        case HI_VI_INTF_MODE_DC:
            (hi_void)memcpy_s(dev_attr, sizeof(hi_vi_dev_attr), &hw_dc_dev_attr, sizeof(hi_vi_dev_attr));
             break;
        default:
            (hi_void)memcpy_s(dev_attr, sizeof(hi_vi_dev_attr), &g_mipi_raw_dev_attr, sizeof(hi_vi_dev_attr));
            break;
...
hi_void sample_comm_vi_get_size_by_sns_type(sample_sns_type sns_type, hi_size *size)
{
    switch (sns_type) {
        ...
        case SAMPLE_VI_MODE_BT1120_1080P_16BIT:
            ...
            break;
        case HW_DC:
            size->width  = HW_WIDTH_1024;
            size->height = HW_HEIGHT_768;
            break;
...
hi_void sample_comm_vi_get_default_sns_info(sample_sns_type sns_type, sample_sns_info *sns_info)
{
    ...
    sns_info->sns_clk_src = 0;//使用sonser0时用0 sonser1时用1
    sns_info->sns_rst_src =  0;//使用sonser0时用0 sonser1时用1
    sns_info->bus_id      = 5; /* 对于hj底板 sensor0 使用4 sensor1 使用5 */
...
hi_void sample_comm_vi_get_default_dev_info(sample_sns_type sns_type, sample_vi_dev_info *dev_info)
{
    ...
    hi_vi_intf_mode intf_mode = HI_VI_INTF_MODE_DC;
    ...
    else if (sns_type == HW_DC ) {
        dev_info->vi_dev = 1;
        intf_mode = HI_VI_INTF_MODE_DC;
    }
...
static hi_void sample_comm_vi_get_default_bind_info(sample_sns_type sns_type, hi_vi_bind_pipe *bind_pipe)
{
    ...
    for (i = 0; i < bind_pipe->pipe_num; i++) {
        ...
        if(sns_type == SAMPLE_VI_MODE_BT1120_1080P_16BIT || sns_type == HW_DC)
        {
                                             ...
                                             sample_vi_pipe_info pipe_info[])
{
    ...
    for (i = 0; i < bind_pipe->pipe_num; i++) {
        ...
        if (sns_type == HW_DC) {
            ...
            pipe_info[i].pipe_attr.compress_mode = HI_COMPRESS_MODE_NONE;
            pipe_info[i].pipe_attr.pixel_format =  OT_PIXEL_FORMAT_YUV_400;
        }
...
//////////////////////////////
        if (i == 0 || (i == 3 && sns_type == SAMPLE_VI_MODE_BT1120_1080P_16BIT) || (i == 3 && sns_type == HW_DC) ) {
        ...
        if(sns_type == SAMPLE_VI_MODE_BT1120_1080P_16BIT)
        {
            pipe_info[i].chn_info[0].chn_attr.pixel_format                   = OT_PIXEL_FORMAT_YUV_400;
        }
        ///////////////
        if(sns_type == HW_DC)
        {
        ...
        {
            pipe_info[i].chn_info[0].chn_attr.pixel_format                   = HI_PIXEL_FORMAT_YUV_400; 
            pipe_info[i].pipe_attr.frame_rate_ctrl.src_frame_rate = 50;
            pipe_info[i].pipe_attr.frame_rate_ctrl.dst_frame_rate = 25;
        }
#endif
    }
}
