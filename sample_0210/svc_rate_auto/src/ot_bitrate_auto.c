...
static hi_s32 clip_3(hi_s32 min, hi_s32 max, hi_s32 val)
{
    val = (val > max) ? max : val;
    val = (val < min) ? min : val;
    return val;
...
static hi_void qpmap_to_deltaqp(hi_u8 qpmap_value, hi_u8 *abs_flag, hi_s8 *qp)
{
    *abs_flag = (qpmap_value >> QPMAP_OFFSET_64) & 1; // 6: 6 bits Convert QpMap value
    ...
    if ((*abs_flag) == 1) {
        ...
        *qp = clip_3(0, 51, (hi_s32)(*qp)); // 51: MaxQp Convert QpMap value
    } else {
...
static hi_void deltaqp_to_qpmap(hi_u8 *qpmap_value, hi_u8 abs_flag, hi_s8 qp)
{
    hi_s32 tmp_qp;
    if (abs_flag == 1) {
...
static hi_s32 add_qp(hi_u8 *qpmap_value, hi_s32 add_value)
{
    hi_u8 abs_flag;
    hi_s8 tmp_qp;
    hi_u8 org_qpmap_value = *qpmap_value;
...
static hi_void adjust_avbr_bitrate(hi_venc_chn_attr *chn_attr, hi_venc_chn chn)
{
    hi_venc_rc_param rc_param;
    static hi_u32 org_min_still_percent = 0;
    hi_s32 tmp_min_still_percent = 0;
    hi_u32 index;
    hi_u32 min_i_qp, min_qp;
    static hi_s32 start_flag = 0;
    if ((chn_attr->rc_attr.rc_mode != HI_VENC_RC_MODE_H264_AVBR) &&
        (chn_attr->rc_attr.rc_mode != HI_VENC_RC_MODE_H265_AVBR)) {
        return;
    }
    if (g_rate_auto_init_param.avbr_rate_control != 0) {
        hi_mpi_venc_get_rc_param(chn, &rc_param);
        if (chn_attr->rc_attr.rc_mode == HI_VENC_RC_MODE_H264_AVBR) {
        ...
        hi_mpi_venc_set_rc_param(chn, &rc_param);
    }
}

static hi_void init_svc_param(hi_venc_svc_param *svc_param)
{
    hi_s32 i;
...
static hi_void adjust_qpmap_based_on_max_qp(hi_s32 max_qpmap_value, hi_venc_svc_param *svc_param)
{
    hi_s32 i;
...
static hi_void adjust_qpmap_based_on_max_bgqp(hi_u32 max_bg_qp, hi_u32 start_qp, hi_venc_svc_param *svc_param)
{
    hi_s32 max_qpmap_value = max_bg_qp - start_qp;
...
static hi_void adjust_qpmap_based_on_min_fgqp(hi_u32 min_fg_qp, hi_u32 start_qp, hi_venc_svc_param *svc_param)
{
    hi_s32 min_qpmap_value;
    hi_s8 tmp_curr_qp;
    hi_u8 tmp_curr_abs;
    hi_s32 i;
...
static hi_void adjust_fg_quality_based_on_max_bgqp(hi_u32 max_bg_qp, hi_u32 start_qp, hi_venc_svc_param *svc_param)
{
    hi_s32 max_qpmap_value;            // Default value
    hi_s32 i;
...
static hi_void adjust_bg_quality_set_value(hi_s32 min_qpmap_value, hi_u8 *qpmap_value)
{
    hi_u8 value = *qpmap_value;
...
static hi_void adjust_bg_quality_based_on_min_fgqp(hi_u32 min_fg_qp, hi_u32 start_qp, hi_venc_svc_param *svc_param)
{
    hi_s32 min_qpmap_value;
...
static hi_void *rate_auto_proc(hi_void *p)
{
    hi_venc_chn_status status;
    hi_venc_svc_param svc_param;
    hi_venc_chn_attr chn_attr;
    rate_auto_thread_attr *para = HI_NULL;
    const hi_venc_chn chn = 0;

    hi_u32 start_qp;
    hi_s32 max_qpmap_value;

    hi_u32 max_bg_qp = g_rate_auto_init_param.max_bg_qp;
    hi_u32 min_fg_qp = g_rate_auto_init_param.min_fg_qp;
    hi_u32 max_fg_qp = g_rate_auto_init_param.max_fg_qp;

    para = (rate_auto_thread_attr *)p;
    hi_mpi_venc_get_chn_attr(chn, &chn_attr);
    hi_mpi_venc_enable_svc(chn, HI_TRUE);

    while (para->start_flag == HI_TRUE) {
        hi_mpi_venc_query_status(chn, &status);
        start_qp = status.stream_info.start_qp;
        hi_mpi_venc_get_svc_param(chn, &svc_param);
        ...
        hi_mpi_venc_set_svc_param(chn, &svc_param);
...
static hi_s32 rate_auto_start_thread(hi_void)
{
    hi_s32 ret;

    g_rate_auto_thread.start_flag = HI_TRUE;
...
static hi_s32 rate_auto_stop_thread(hi_void)
{
    if (g_rate_auto_thread.start_flag == HI_TRUE) {
        g_rate_auto_thread.start_flag = HI_FALSE;
...
hi_s32 hi_rate_auto_init(const rate_auto_param *init_param)
{
    ...
    (hi_void)memcpy_s(&g_rate_auto_init_param, sizeof(g_rate_auto_init_param), init_param, sizeof(rate_auto_param));
...
hi_s32 hi_rate_auto_deinit(hi_void)
{
    ...
    (hi_void)memset_s(&g_rate_auto_init_param, sizeof(rate_auto_param), 0, sizeof(rate_auto_param));
