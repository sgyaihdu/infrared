...
static struct hi_uvc_dev *g_hi_cd = 0;

static const hi_char *image_fmt_to_string(hi_u32 format)
{
    switch (format) {
...
#ifdef HI_DEBUG
static const hi_char *get_code(hi_s32 code)
{
...
static const hi_char *get_intf_cs_string(hi_s32 code)
{
    if (code == 0x01) {
        return "PROB_CONTROL";
    } else if (code == 0x02) {
...
static struct hi_uvc_dev *hi_uvc_init(const hi_char *dev_name)
{
    struct hi_uvc_dev *uvc_dev = NULL;
    struct video_ability cap;
    hi_s32 ret;
    hi_s32 fd;

    hi_char *regular_path = realpath(dev_name, NULL);
    ...
    uvc_dev = (struct hi_uvc_dev *)malloc(sizeof *uvc_dev);
    if (uvc_dev == NULL) {
        close(fd);
        return NULL;
    }

    (hi_void)memset_s(uvc_dev, sizeof(*uvc_dev), 0, sizeof(*uvc_dev));
    uvc_dev->i_fd = fd;

    return uvc_dev;
}

static hi_s32 hi_uvc_video_process_user_ptr(struct hi_uvc_dev *uvc_dev)
{
    struct video_cache v_cache;
    hi_s32 ret;

    (hi_void)memset_s(&v_cache, sizeof(v_cache), 0, sizeof(v_cache));
...
static hi_void hi_uvc_video_stream_user_ptr(struct hi_uvc_dev *uvc_dev)
{
    struct video_cache v_cache;
    hi_s32 cache_type = VIDEO_CACHE_TYPE_OPT;
    hi_s32 ret;
...
static hi_s32 hi_uvc_video_set_fmt(struct hi_uvc_dev *uvc_dev)
{
    struct video_fmt v_fmt;
    hi_s32 ret;

    (hi_void)memset_s(&v_fmt, sizeof(v_fmt), 0, sizeof(v_fmt));
...
static hi_void hi_uvc_stream_off(struct hi_uvc_dev *uvc_dev)
{
    hi_s32 cache_type = VIDEO_CACHE_TYPE_OPT;
    hi_s32 ret;
...
static hi_void hi_uvc_video_disable(struct hi_uvc_dev *uvc_dev)
{
    hi_uvc_stream_off(uvc_dev);
}

static hi_void hi_uvc_video_enable(struct hi_uvc_dev *uvc_dev)
{
    encoder_property ep;

    hi_uvc_video_disable(uvc_dev);
    ...
    hi_uvc_video_stream_user_ptr(uvc_dev);
}

static hi_s32 is_contain_udc_sub_dir(const hi_char *path, hi_s32 path_size)
{
    DIR *p_dir = NULL;
...
static hi_s32 read_file(const hi_char *path, hi_u32 path_size, const hi_char *dest, hi_u32 size)
{
    FILE *input = NULL;
    hi_char *realpath_res = NULL;
    hi_s32 ret;
...
static hi_s32 get_udc_node_name(hi_char *node_name, hi_u32 size)
{
    DIR *p_dir = NULL;
    struct dirent *p_dirent = NULL;
    const hi_char *path_tmp = "/sys/kernel/config/usb_gadget/";
    hi_char tmp[UDC_PATH_LENGTH]; /* 389: 256 bytes used for d_name, 128 bytes used for path_tmp, 5 bytes for UDC */
...
static hi_u32 get_max_payload_transfer_size(const struct hi_uvc_frame_info *frm_info)
{
    const hi_u32 high_speed_size = 3072;
    const hi_u32 full_speed_size = 1023;
    const hi_char *path_tmp = "/sys/class/udc/";

    hi_char tmp[128];
    hi_char target_path[GET_MAX_PACKET_SIZE_PATH]; // 398=256+128+14
    hi_u32 result = high_speed_size;
...
static hi_void hi_uvc_get_buf_size(struct hi_uvc_dev *uvc_dev, const struct hi_uvc_frame_info *frm_info, hi_u32 fcc,
...
static hi_void hi_uvc_handle_streaming_control(struct hi_uvc_dev *uvc_dev, struct hi_uvc_streaming_control *u_str_ctrl,
    hi_s32 i_frame, hi_s32 i_format)
{
    const struct hi_uvc_format_info *fmt_info = NULL;
    const struct hi_uvc_frame_info *frm_info = NULL;
    hi_u32 n_frm = 0;
    hi_u32 max_video_frame_size = 0;

    if (i_format < 0) {
        i_format = (hi_s32)(array_size(hi_fmt) + i_format);
    }

    if ((i_format < 0) || (i_format >= (hi_s32)array_size(hi_fmt))) {
        return;
    }

    debug("i_format = %d\n", i_format);
    fmt_info = &hi_fmt[i_format];
    ...
    if ((i_frame < 0) || (i_frame >= (hi_s32)n_frm)) {
        return;
    }

    frm_info = &fmt_info->frames[i_frame];

    (hi_void)memset_s(u_str_ctrl, sizeof(*u_str_ctrl), 0, sizeof(*u_str_ctrl));
    ...
    hi_uvc_get_buf_size(uvc_dev, frm_info, fmt_info->fcc, &max_video_frame_size);
    (hi_void)memcpy_s(&u_str_ctrl->dw_max_video_frame_size, sizeof(hi_u32), &max_video_frame_size, sizeof(hi_u32));
...
static hi_void hi_uvc_handle_standard_request(const struct hi_uvc_dev *uvc_dev,
...
static void hi_uvc_eve_undef_control(struct hi_uvc_dev *uvc_dev, hi_u8 cs, struct uvc_request_data *u_req_data)
{
    switch (cs) {
...
static hi_void hi_uvc_handle_control_request(struct hi_uvc_dev *uvc_dev, hi_u8 req, hi_u8 unit_id, hi_u8 cs,
    struct uvc_request_data *u_req_data)
{
    switch (unit_id) {
        case OT_UVC_VC_DESCRIPTOR_UNDEFINED:
            hi_uvc_eve_undef_control(uvc_dev, cs, u_req_data);
            break;
...
static hi_void hi_uvc_handle_streaming_request(struct hi_uvc_dev *uvc_dev, hi_u8 req, hi_u8 cs,
    struct uvc_request_data *u_req_data)
{
    struct hi_uvc_streaming_control u_str_ctrl;
    struct hi_uvc_streaming_control *str_ctl;
    ...
    u_req_data->length = (hi_s32)sizeof(u_str_ctrl);
    const struct hi_uvc_format_info *format = &hi_fmt[uvc_dev->probe.b_format_index - 1];
    const struct hi_uvc_frame_info *frame = &format->frames[uvc_dev->probe.b_frame_index - 1];
    ...
    switch (req) {
        ...
        case OT_UVC_GET_CUR:
            ...
            (hi_void)memcpy_s(&u_str_ctrl, sizeof(u_str_ctrl), str_ctl, sizeof(u_str_ctrl));
            u_str_ctrl.dw_max_payload_transfer_size = get_max_payload_transfer_size(frame);
            break;
        case OT_UVC_GET_MIN:
        case OT_UVC_GET_MAX:
        case OT_UVC_GET_DEF:
            hi_uvc_handle_streaming_control(uvc_dev, &u_str_ctrl, uvc_dev->probe.b_frame_index - 1, \
                uvc_dev->probe.b_format_index - 1);
            break;
        case OT_UVC_GET_RES:
            (hi_void)memset_s(u_req_data->data, sizeof(u_req_data->data), 0, sizeof(u_req_data->data));
            return;
        case OT_UVC_GET_LEN:
...
static hi_void set_probe_status(struct hi_uvc_dev *uvc_dev, hi_s32 cs, hi_s32 req)
{
    if (cs == 0x01) {
        switch (req) {
            case 0x01:
...
static hi_void hi_uvc_handle_class_request(struct hi_uvc_dev *uvc_dev, struct usb_ctrlrequest *u_ctrl_req,
    struct uvc_request_data *u_req_data)
{
    const hi_bool probe_status = HI_TRUE;

    if (probe_status) {
        hi_u8 recip_type = u_ctrl_req->bRequestType & USB_RECIP_MASK;
        switch (recip_type) {
            case USB_RECIP_INTERFACE:
                debug("request type :hi_s32ERFACE\n");
                ...
                debug("cs code : 0x%02x(%s)\n", (u_ctrl_req->wValue >> RIGHT_SHIFT_8BIT),
                    (hi_char *)get_intf_cs_string((u_ctrl_req->wValue >> RIGHT_SHIFT_8BIT)));
                debug("req code: 0x%02x(%s)\n", u_ctrl_req->bRequest, (hi_char *)get_code(u_ctrl_req->bRequest));
    ...
    switch (u_ctrl_req->wIndex & 0xff) {
        case OT_UVC_INTF_CONTROL:
            hi_uvc_handle_control_request(uvc_dev, u_ctrl_req->bRequest, u_ctrl_req->wIndex >> RIGHT_SHIFT_8BIT,
                u_ctrl_req->wValue >> RIGHT_SHIFT_8BIT, u_req_data);
            break;
        case OT_UVC_INTF_STREAMING:
            hi_uvc_handle_streaming_request(uvc_dev, u_ctrl_req->bRequest, u_ctrl_req->wValue >> RIGHT_SHIFT_8BIT,
                u_req_data);
            break;
        default:
            break;
...
static hi_void do_hi_uvc_setup_event(struct hi_uvc_dev *uvc_dev, struct usb_ctrlrequest *u_ctrl_req,
    struct uvc_request_data *u_req_data)
{
    uvc_dev->i_control = 0;
    uvc_dev->i_unit_id = 0;
    ...
    switch (u_ctrl_req->bRequestType & USB_TYPE_MASK) {
        case USB_TYPE_STANDARD:
            hi_uvc_handle_standard_request(uvc_dev, u_ctrl_req, u_req_data);
            break;
        case USB_TYPE_CLASS:
            hi_uvc_handle_class_request(uvc_dev, u_ctrl_req, u_req_data);
            break;
        default:
            break;
    }
...
static hi_void handle_control_interface_data(struct hi_uvc_dev *uvc_dev, struct uvc_request_data *u_req_data)
{
...
static hi_u32 clamp(hi_u32 val, hi_u32 min, hi_u32 max)
{
    hi_u32 res = val;
    if (res < min) {
        return min;
...
static void compute_video_frame_size(hi_u32 img_fmt, const struct hi_uvc_dev *uvc_dev,
    struct hi_uvc_streaming_control *u_str_target, const struct hi_uvc_frame_info *frm_info)
{
    switch (img_fmt) {
        case VIDEO_IMG_FORMAT_YUYV:
...
static struct hi_uvc_streaming_control *get_streaming_control(hi_s32 control_type, struct hi_uvc_dev *uvc_dev,
...
static hi_void do_hi_uvc_data_event(struct hi_uvc_dev *uvc_dev, struct uvc_request_data *u_req_data)
{
    struct hi_uvc_streaming_control *u_str_target = NULL;
    struct hi_uvc_streaming_control u_str_ctrl;
    const struct hi_uvc_format_info *fmt_info = NULL;
    const struct hi_uvc_frame_info *frm_info = NULL;
    const hi_u32 *intval = NULL;
    hi_u32 i_fmt, i_frm;
    hi_u32 n_frm;
    ...
    hi_s32 ret = memcpy_s(&u_str_ctrl, sizeof(u_str_ctrl), u_req_data->data, sizeof(u_str_ctrl));
    if (ret != EOK) {
        ERR_INFO("memcpy_s u_str_ctrl fail, ret %d\n", ret);
        return;
    }

    i_fmt = clamp((hi_u32)u_str_ctrl.b_format_index, 1U, (hi_u32)array_size(hi_fmt));

    fmt_info = &hi_fmt[i_fmt - 1];
    n_frm = 0;
    ...
    i_frm = clamp((hi_u32)u_str_ctrl.b_frame_index, 1U, n_frm);
    ...
    u_str_target->b_format_index = (hi_u8)i_fmt;
    u_str_target->b_frame_index = (hi_u8)i_frm;
    ...
    if (uvc_dev->i_control == OT_UVC_VS_COMMIT_CONTROL) {
        ...
        hi_uvc_video_set_fmt(uvc_dev);

        if (uvc_dev->dw_bulk != 0) {
            hi_uvc_video_disable(uvc_dev);
            hi_uvc_video_enable(uvc_dev);
        }
        (hi_void)memset_s(&uvc_dev->probe_status, sizeof(uvc_dev->probe_status), 0, sizeof(uvc_dev->probe_status));
    }
}

static hi_void do_hi_uvc_event(struct hi_uvc_dev *uvc_dev)
{
    struct video_event v_event;
    struct hi_uvc_event *hi_eve = (struct hi_uvc_event *)(hi_void *)&v_event.u.a_data;
    struct uvc_request_data u_req_data;
    hi_s32 ret;

    debug("#############do_hi_uvc_event()#############\n");
    ...
    (hi_void)memset_s(&u_req_data, sizeof(u_req_data), 0, sizeof(u_req_data));
    u_req_data.length = 32; /* 32 byte data length */

    switch (v_event.dw_type) {
        case OT_UVC_EVE_CON:
            debug("handle connect event\n");
            hi_uvc_handle_streaming_control(uvc_dev, &uvc_dev->probe, 0, 0);
            hi_uvc_handle_streaming_control(uvc_dev, &uvc_dev->commit, 0, 0);
            return;
        case OT_UVC_EVE_DISCON:
            return;
        case OT_UVC_EVE_SETTING:
            do_hi_uvc_setup_event(uvc_dev, &hi_eve->req, &u_req_data);
            break;
        case OT_UVC_EVE_DATA:
            do_hi_uvc_data_event(uvc_dev, &hi_eve->data);
            return;
...
static hi_void hi_uvc_event_register(struct hi_uvc_dev *uvc_dev)
{
    struct video_event_descriptor v_event_desc;
    hi_s32 ret;

    (hi_void)memset_s(&v_event_desc, sizeof(v_event_desc), 0, sizeof(v_event_desc));
...
{
    struct hi_uvc_dev *uvc_dev;

    uvc_dev = hi_uvc_init(dev_path);
    if (uvc_dev == 0) {
        return -1;
    }
...
hi_s32 run_uvc_data(hi_void)
{
    ...
    if (g_hi_cd->i_streaming == 1) {
        FD_SET(g_hi_cd->i_fd, &w_fds);
    }

    ret = select(g_hi_cd->i_fd + 1, NULL, &w_fds, NULL, &t_val);
    if (ret > 0) {
        if (FD_ISSET(g_hi_cd->i_fd, &w_fds)) {
            hi_uvc_video_process_user_ptr(g_hi_cd);
        }
    }
...
hi_s32 run_uvc_device(hi_void)
{
    ...
    FD_ZERO(&e_fds);
    FD_SET(g_hi_cd->i_fd, &e_fds);

    ret = select(g_hi_cd->i_fd + 1, NULL, NULL, &e_fds, &t_val);
    if (ret > 0) {
        if (FD_ISSET(g_hi_cd->i_fd, &e_fds)) {
            do_hi_uvc_event(g_hi_cd);
        }
    }
