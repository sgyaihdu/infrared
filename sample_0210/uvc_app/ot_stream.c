...
static struct ot_stream g_hi_stream = {
    .mpi_sc_ops = NULL,
    .mpi_pu_ops = NULL,
    .mpi_it_ops = NULL,
    .mpi_eu_ops = NULL,
    .streaming = 0,
    .exposure_auto_stall = HI_FALSE,
    .brightness_stall = HI_FALSE,
};
 ...
 * ***************************************************** */
static void brightness_set_cur(struct hi_uvc_dev *dev, struct uvc_request_data *resp)
{
    resp->length = OT_UVC_RESP_LENGTH_HALF_WORD;
...
static void brightness_get_cur(struct hi_uvc_dev *dev, struct uvc_request_data *resp, uint8_t v)
{
    if (get_ot_stream()->brightness_stall) {
        resp->length = -EL2HLT;
        dev->request_error_code.data[0] = 0x04;
        dev->request_error_code.length = 1;
        get_ot_stream()->brightness_stall = HI_FALSE;
    } else {
        resp->length = OT_UVC_RESP_LENGTH_HALF_WORD;
        resp->data[0] = v;
        resp->data[1] = 0;
...
static void brightness_get_min(struct hi_uvc_dev *dev, struct uvc_request_data *resp)
{
    resp->length = OT_UVC_RESP_LENGTH_HALF_WORD;
    resp->data[0] = 0x00;
    dev->request_error_code.data[0] = 0x00;
...
static void brightness_get_max(struct hi_uvc_dev *dev, struct uvc_request_data *resp)
{
    resp->length = OT_UVC_RESP_LENGTH_HALF_WORD;
    resp->data[0] = 0x64;
    dev->request_error_code.data[0] = 0x00;
...
static void brightness_get_def(struct hi_uvc_dev *dev, struct uvc_request_data *resp)
{
    resp->length = OT_UVC_RESP_LENGTH_HALF_WORD;
    resp->data[0] = 0x32;
    dev->request_error_code.data[0] = 0x00;
...
static void brightness_get_res(struct hi_uvc_dev *dev, struct uvc_request_data *resp)
{
    resp->data[0] = 0x01;
    resp->length = OT_UVC_RESP_LENGTH_HALF_WORD;
    dev->request_error_code.data[0] = 0x00;
...
static void brightness_get_info(struct hi_uvc_dev *dev, struct uvc_request_data *resp)
{
    resp->data[0] = 0x03;
    resp->length = OT_UVC_RESP_LENGTH_BYTE;
    dev->request_error_code.data[0] = 0x00;
...
static void ot_stream_pu_brightness_ctrl(struct hi_uvc_dev *dev, uint8_t req, struct uvc_request_data *resp)
{
    uint16_t v = 50;
...
static void ot_stream_pu_brightness_set(struct uvc_request_data *data)
{
    ...
    if (v > 0x64) {
        get_ot_stream()->brightness_stall = HI_TRUE;
    }
...
static void ot_stream_pu_contrast_ctrl(struct hi_uvc_dev *dev, uint8_t req, struct uvc_request_data *resp)
{
...
static void ot_stream_pu_hue_ctrl(struct hi_uvc_dev *dev, uint8_t req, struct uvc_request_data *resp)
{
    uint16_t v = 50; /* 50 is in range of 0~100  */
...
static void ot_stream_pu_power_line_frequency_ctrl(struct hi_uvc_dev *dev, uint8_t req, struct uvc_request_data *resp)
{
    uint8_t v = 0;
...
static void ot_stream_pu_saturation_ctrl(struct hi_uvc_dev *dev, uint8_t req, struct uvc_request_data *resp)
{
    uint16_t v = 50; /* 50 is in range of 0~100  */
...
static void ot_stream_pu_white_balance_temperature_auto_ctrl(struct hi_uvc_dev *dev, uint8_t req,
...
static void ot_stream_pu_white_balance_temperature_ctrl(struct hi_uvc_dev *dev, uint8_t req,
    struct uvc_request_data *resp)
{
...
void ot_stream_event_pu_control(struct hi_uvc_dev *dev, uint8_t req, uint8_t unit_id, uint8_t cs,
    struct uvc_request_data *resp)
{
...
void ot_stream_event_pu_data(struct hi_uvc_dev *dev, int unit_id, int control, struct uvc_request_data *data)
{
    if (data == NULL) {
        err("data is NULL\n");
        return;
...
static void exposure_auto_set_cur(struct hi_uvc_dev *dev, struct uvc_request_data *resp)
{
    resp->data[0] = 0x0;
    resp->length = OT_UVC_RESP_LENGTH_BYTE;
    dev->request_error_code.data[0] = 0x00;
...
static void exposure_auto_get_cur(struct hi_uvc_dev *dev, struct uvc_request_data *resp, uint8_t v)
{
    if (get_ot_stream()->exposure_auto_stall) {
        resp->length = -EL2HLT;
        dev->request_error_code.data[0] = 0x04;
        dev->request_error_code.length = 1;
        get_ot_stream()->exposure_auto_stall = HI_FALSE;
    } else {
        resp->data[0] = v;
        resp->length = OT_UVC_RESP_LENGTH_BYTE;
        dev->request_error_code.data[0] = 0x00;
...
static void exposure_auto_get_res(struct hi_uvc_dev *dev, struct uvc_request_data *resp)
{
    resp->data[0] = 0x05;
    resp->length = OT_UVC_RESP_LENGTH_BYTE;
    dev->request_error_code.data[0] = 0x00;
...
static void exposure_auto_get_def(struct hi_uvc_dev *dev, struct uvc_request_data *resp)
{
    resp->data[0] = 0x04;
    resp->length = OT_UVC_RESP_LENGTH_BYTE;
    dev->request_error_code.data[0] = 0x00;
...
static void exposure_auto_get_info(struct hi_uvc_dev *dev, struct uvc_request_data *resp)
{
    resp->data[0] = 0x3;
    resp->length = OT_UVC_RESP_LENGTH_BYTE;
    dev->request_error_code.data[0] = 0x00;
 ...
 * ***************************************************** */
static void ot_stream_it_exposure_auto_mode_ctrl(struct hi_uvc_dev *dev, uint8_t req, struct uvc_request_data *resp)
{
    uint8_t v = 0x04;
...
static void ot_stream_it_exposure_auto_mode_set(struct hi_uvc_dev *dev, struct uvc_request_data *data)
{
    uint8_t v = 0x04;
    ...
    switch (v) {
        ...
        case 0x08:
            ...
            break;
        default:
            get_ot_stream()->exposure_auto_stall = HI_TRUE;
            return;
    }
...
static void exposure_time_get_cur(struct uvc_request_data *resp, uint32_t v, struct hi_uvc_dev *dev)
{
    resp->length = OT_UVC_RESP_LENGTH_WORD;
    resp->data[0] = (v & 0xff);
    resp->data[1] = ((v >> SHIFT_8BIT) & 0xff);
...
static void ot_stream_it_exposure_ansolute_time_ctrl(struct hi_uvc_dev *dev, uint8_t req,
    struct uvc_request_data *resp)
{
...
void ot_stream_event_it_control(struct hi_uvc_dev *dev, uint8_t req, uint8_t unit_id, uint8_t cs,
    struct uvc_request_data *resp)
{
    if (dev == NULL || resp == NULL) {
        err("dev or resp is NULL\n");
...
void ot_stream_event_it_data(struct hi_uvc_dev *dev, int unit_id, int control, struct uvc_request_data *data)
{
...
void ot_stream_event_eu_h264_control(struct hi_uvc_dev *dev, uint8_t req, uint8_t unit_id, uint8_t cs,
    struct uvc_request_data *resp)
{
    if (resp == NULL) {
        err("resp is NULL\n");
...
void ot_stream_event_eu_h264_data(struct hi_uvc_dev *dev, int unit_id, int control, struct uvc_request_data *data)
{
    if (unit_id != 10) { /* 10 == id of xu */
        return;
    }
...
void ot_stream_event_eu_camera_control(struct hi_uvc_dev *dev, uint8_t req, uint8_t unit_id, uint8_t cs,
    struct uvc_request_data *resp)
{
    if (resp == NULL) {
        err("resp is NULL\n");
...
void ot_stream_event_eu_camera_data(struct hi_uvc_dev *dev, int unit_id, int control, struct uvc_request_data *data)
{
    if (unit_id != UNIT_XU_OT_CAMERA) {
        return;
    }
