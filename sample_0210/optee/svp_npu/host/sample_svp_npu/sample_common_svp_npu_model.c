...
hi_void sample_common_svp_npu_unload_model(hi_u32 model_index)
{
    ...
    sample_svp_check_exps_return(g_svp_npu_model[model_index].is_load_flag != HI_TRUE, (hi_void)0,
...
static hi_s32 sample_svp_npu_malloc_mem(hi_void **buffer, hi_u32 buffer_size, hi_bool is_cached, hi_ulong class_id)
{
    svp_acl_error ret;
...
static hi_void sample_svp_npu_destroy_data_buffer(svp_acl_data_buffer *input_data, td_ulong class_id)
{
    hi_void *data = svp_acl_get_data_buffer_addr(input_data);
    if (class_id == 0) {
        (hi_void)svp_acl_rt_free(data);
    } else {
        (hi_void)svp_acl_rt_free_sec(data);
    }
    ...
    hi_ulong *class_id)
{
    ...
    sample_svp_check_exps_return(ret != HI_SUCCESS, input_data, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error, %u-th svp_acl_mdl_get_input_class_id_by_index failed!\n", idx);

    if (sample_svp_npu_malloc_mem(&input_buffer, (hi_u32)buffer_size, task->cfg.is_cached, *class_id) != HI_SUCCESS) {
    ...
    if (input_data == HI_NULL) {
        ...
        if (*class_id == 0) {
            ...
            (hi_void)svp_acl_rt_free_sec(input_buffer);
        }
        return input_data;
    }
    if (idx == g_svp_npu_model[task->cfg.model_idx].input_num - SAMPLE_SVP_NPU_EXTRA_INPUT_NUM) {
    ...
    hi_u32 idx, hi_ulong *class_id)
{
    ...
    sample_svp_check_exps_return(ret != HI_SUCCESS, output_data, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error, %u-th svp_acl_mdl_get_output_class_id_by_index failed!\n", idx);

    if (sample_svp_npu_malloc_mem(&output_buffer, buffer_size, task->cfg.is_cached, *class_id) != HI_SUCCESS) {
    ...
    if (output_data == HI_NULL) {
        ...
        if (*class_id == 0) {
            ...
            (hi_void)svp_acl_rt_free_sec(output_buffer);
        }
        return output_data;
    }
    return output_data;
...
hi_s32 sample_common_svp_npu_create_input(sample_svp_npu_task_info *task)
{
    ...
    for (i = 0; i < g_svp_npu_model[task->cfg.model_idx].input_num - SAMPLE_SVP_NPU_EXTRA_INPUT_NUM; i++) {
        ...
        if (ret != SVP_ACL_SUCCESS) {
            ...
            (hi_void)sample_svp_npu_destroy_data_buffer(input_data, class_id);
            (hi_void)sample_common_svp_npu_destroy_input(task);
            return HI_FAILURE;
        }
    }
...
hi_s32 sample_common_svp_npu_create_output(sample_svp_npu_task_info *task)
{
    ...
    for (i = 0; i < g_svp_npu_model[task->cfg.model_idx].output_num; i++) {
        ...
        if (ret != SVP_ACL_SUCCESS) {
            ...
            (hi_void)sample_svp_npu_destroy_data_buffer(output_data, class_id);
    ...
    for (i = 0; i < input_num; i++) {
        ...
        if (i < g_svp_npu_model[task->cfg.model_idx].input_num - SAMPLE_SVP_NPU_EXTRA_INPUT_NUM) {
            ret = svp_acl_mdl_get_input_class_id_by_index(g_svp_npu_model[task->cfg.model_idx].model_desc, i,
                &class_id);
            sample_svp_check_exps_return(ret != HI_SUCCESS, (hi_void)0, SAMPLE_SVP_ERR_LEVEL_ERROR,
                "Error, %u-th svp_acl_mdl_get_input_class_id_by_index failed!\n", i);
            data = svp_acl_get_data_buffer_addr(data_buffer);
            if (class_id == 0) {
                (hi_void)svp_acl_rt_free(data);
...
hi_void sample_common_svp_npu_destroy_work_buf(sample_svp_npu_task_info *task)
{
    ...
    if (class_id == 0) {
        ...
        (hi_void)svp_acl_rt_free_sec(task->work_buf_ptr);
    }
...
hi_s32 sample_common_svp_npu_create_task_buf(sample_svp_npu_task_info *task)
{
    ...
    if (ret != SVP_ACL_SUCCESS) {
        ...
        (hi_void)sample_svp_npu_destroy_data_buffer(task_buf, 0);
        return HI_FAILURE;
    }
    return HI_SUCCESS;
}
...
hi_s32 sample_common_svp_npu_create_work_buf(sample_svp_npu_task_info *task)
{
    ...
    if (ret != SVP_ACL_SUCCESS) {
        ...
        (hi_void)sample_svp_npu_destroy_data_buffer(work_buf, class_id);
        return HI_FAILURE;
    }
    return HI_SUCCESS;
}
...
hi_void sample_common_svp_npu_output_classification_result(const sample_svp_npu_task_info *task)
{
    ...
    sample_svp_check_exps_return(task->output_dataset == HI_NULL, (hi_void)0, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "task->output_dataset is NULL!\n");

    for (i = 0; i < svp_acl_mdl_get_dataset_num_buffers(task->output_dataset); i++) {
        data_buffer = svp_acl_mdl_get_dataset_buffer(task->output_dataset, i);
        sample_svp_check_exps_return(data_buffer == HI_NULL, (hi_void)0, SAMPLE_SVP_ERR_LEVEL_ERROR,
            "get %u-th data buffer is NULL!\n", i);

        data = svp_acl_get_data_buffer_addr(data_buffer);
        sample_svp_check_exps_return(data == HI_NULL, (hi_void)0, SAMPLE_SVP_ERR_LEVEL_ERROR,
            "get %u-th data addr is NULL!\n", i);

        ret = svp_acl_mdl_get_output_class_id_by_index(g_svp_npu_model[task->cfg.model_idx].model_desc, i, &class_id);
        sample_svp_check_exps_return((ret != HI_SUCCESS || class_id != 0), (hi_void)0, SAMPLE_SVP_ERR_LEVEL_ERROR,
    ...
    hi_bool is_input)
{
    ...
    for (i = 0; i < data_num; i++) {
        ...
        stride = svp_acl_get_data_buffer_stride(data_buffer);
        sample_svp_check_exps_return(stride == 0, (hi_void)0, SAMPLE_SVP_ERR_LEVEL_ERROR,
            "get %u-th data stride failed!\n", i);

        size = svp_acl_get_data_buffer_size(data_buffer);
        sample_svp_check_exps_return(size == 0, (hi_void)0, SAMPLE_SVP_ERR_LEVEL_ERROR,
            "get %u-th data size failed!\n", i);

        sample_svp_check_exps_return(size < (hi_u64)task->cfg.dynamic_batch_num * total_line_num * stride, (hi_void)0,
            SAMPLE_SVP_ERR_LEVEL_ERROR, "%u-th data buffer size(%lu) is less than needed(%llu)!\n",
            i, size, (hi_u64)task->cfg.dynamic_batch_num * total_line_num * stride);

        ret = snprintf_s(name, SAMPLE_SVP_NPU_MAX_NAME_LEN - 1, SAMPLE_SVP_NPU_MAX_NAME_LEN - 1, file_name, i);
        sample_svp_check_exps_return(ret < 0, (hi_void)0, SAMPLE_SVP_ERR_LEVEL_ERROR, "create file name failed!\n");

        fp = fopen(name, "wb");
        sample_svp_check_exps_return(fp == HI_NULL, (hi_void)0, SAMPLE_SVP_ERR_LEVEL_ERROR, "open file failed!\n");

        for (line = 0; line < task->cfg.dynamic_batch_num * total_line_num; line++) {
            ret = fwrite(data, line_byte_num, 1, fp);
            if (ret != 1) {
                fclose(fp);
                sample_svp_check_exps_return(ret != 1, (hi_void)0, SAMPLE_SVP_ERR_LEVEL_ERROR, "write file failed!\n");
            }
            data += stride;
        }
        (hi_void)fclose(fp);
...
hi_void sample_common_svp_npu_dump_task_data(const sample_svp_npu_task_info *task)
{
    ...
    sample_svp_check_exps_return(task->input_dataset == HI_NULL, (hi_void)0, SAMPLE_SVP_ERR_LEVEL_ERROR,
