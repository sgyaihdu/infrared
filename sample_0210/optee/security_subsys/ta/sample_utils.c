...
td_void cipher_free(crypto_buf_attr *buf_attr, void *virt_addr)
{
    if (buf_attr->phys_addr != TD_NULL && virt_addr != TD_NULL) {
        mpi_sys_mmz_free(buf_attr->phys_addr, virt_addr);
    }
}
...
td_s32 cipher_alloc(crypto_buf_attr *buf_attr, void **virt_addr, unsigned int size)
{
    ...
    ret = mpi_sys_mmz_alloc(&(buf_attr->phys_addr), virt_addr, "SAMPLE Buffer", "SMMZ2", size);
    if (ret != TD_SUCCESS) {
