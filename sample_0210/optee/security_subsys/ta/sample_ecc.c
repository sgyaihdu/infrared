...
static td_s32 run_eddsa_sign_sample(ecc_data *data)
{
    ...
    /* 3. EDDSA sign */
    sample_chk_expr_goto(ot_mpi_cipher_pke_eddsa_sign(data->curve_type, &priv_key, &msg, &output_sig),
        TD_SUCCESS, __PKE_DEINIT__);

    /* 4. EDDSA verify. */
    sample_chk_expr_goto(ot_mpi_cipher_pke_eddsa_verify(data->curve_type, &pub_key, &msg, &output_sig),
        TD_SUCCESS, __PKE_DEINIT__);
