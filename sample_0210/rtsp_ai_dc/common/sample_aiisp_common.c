...
static sample_comm_venc_chn_param g_venc_chn_param = {
    ...
    .stats_time           = 1,  /* 2 is a number */
    .gop                  = 25, /* 60 is a number */
...
static hi_void sample_aiisp_init_mem_share(hi_void)
{
    hi_u32 i;
...
hi_s32 sample_aiisp_sys_init(hi_size *in_size, hi_u32 vb_cnt)
{
    ...
    if(in_size->width == 512 && in_size->height == 768)
    {
        hi_size hw_dc_size = {1024,768};
        sample_aiisp_get_default_vb_config(&hw_dc_size, &vb_cfg, vb_cnt);
...
hi_s32 sample_aiisp_start_venc(hi_venc_chn venc_chn[], hi_u32 venc_chn_len, hi_u32 chn_num, hi_size *size)
{
    ...
    sample_print("venc_size = %d, width = %d ,height = %d\n",\
    g_venc_chn_param.size,g_venc_chn_param.venc_size.width,\
    g_venc_chn_param.venc_size.height);
#if 0
    hi_venc_mod_param mod_param = {0};
    mod_param.mod_type = OT_VENC_MOD_H265;
    hi_mpi_venc_get_mod_param(&mod_param);
    mod_param.h265_mod_param.one_stream_buf = 1;
    hi_mpi_venc_set_mod_param(&mod_param);
#endif
...
hi_s32 sample_aiisp_start_venc_and_vo(hi_vpss_grp vpss_grp[], hi_u32 grp_num, hi_size *in_size)
{
    ...
    #if 1
    hi_size rtsp_size = {AIISP_VENC_WIDTH, AIISP_VENC_HEIGHT};
    ret = sample_aiisp_start_venc(venc_chn, sizeof(venc_chn) / sizeof(hi_venc_chn), grp_num, &rtsp_size);
    if (ret != HI_SUCCESS) {
