...
hi_void cipher_free(const crypto_buf_attr *buf_attr, const void *virt_addr)
{
...
hi_s32 get_random_data(hi_u8 *buffer, hi_u32 size)
{
    hi_s32 ret;
    if (buffer == HI_NULL) {
    ...
    ret = hi_mpi_cipher_trng_get_multi_random(size, buffer);
    return ret;
}

static hi_s32 get_rsa3072_key(drv_pke_rsa_pub_key *pub_key, drv_pke_rsa_priv_key *priv_key)
{
...
static hi_s32 get_rsa4096_key(drv_pke_rsa_pub_key *pub_key, drv_pke_rsa_priv_key *priv_key)
{
    to_be_processed(pub_key);
    to_be_processed(priv_key);
...
hi_s32 get_rsa_key(drv_pke_rsa_pub_key *pub_key, drv_pke_rsa_priv_key *priv_key, hi_u32 key_len)
{
    hi_s32 ret = HI_SUCCESS;
    if (pub_key == HI_NULL || priv_key == HI_NULL) {
        return HI_FAILURE;
    }
    switch (key_len) {
        case RSA_3072_KEY_LEN:
            sample_chk_expr_return(get_rsa3072_key(pub_key, priv_key), HI_SUCCESS);
            break;
        case RSA_4096_KEY_LEN:
            sample_chk_expr_return(get_rsa4096_key(pub_key, priv_key), HI_SUCCESS);
            break;
        default:
...
hi_void destroy_rsa_key(drv_pke_rsa_pub_key *pub_key, drv_pke_rsa_priv_key *priv_key, hi_u32 key_len)
{
    to_be_processed(pub_key);
    to_be_processed(priv_key);
