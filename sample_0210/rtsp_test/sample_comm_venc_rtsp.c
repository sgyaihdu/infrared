...
#define RTSP_FRAME_RATE 25

#define SRC_FRAME_RATE 30
#if ZLMEDIAKIT_IS_INIT
...
static td_u8* g_frame_buf = TD_NULL;
static td_u32 g_frame_buf_size = (1936*1280*3/2);
...
#define EXPECTED_DELTA (unsigned int )(90000.0 / 30) // 根据帧率30fps计算

static td_u32 g_time_pts = 0;
 
static td_u32 g_time_pts_delta = EXPECTED_DELTA;
#endif
    ...
    hi_venc_stream_buf_info *stream_buf_info, hi_payload_type *payload_type)
{
    ...
    // frame_buf = (td_u8 *)malloc(frame_len*sizeof(td_u8));
    // if(frame_buf == NULL)
    // {
    //     sample_print("malloc error\n");
    //     free(stream.pack);
    ...
    for (i = 0; i < stream.pack_cnt; i++)
    {
        //sample_print("frame_input\n");
        memcpy(g_frame_buf + offset, stream.pack[i].addr+stream.pack[i].offset, stream.pack[i].len-stream.pack[i].offset);
        offset += (stream.pack[i].len-stream.pack[i].offset);
    }

    //printf("Frame size: %d, PTS: %llu\n", frame_len, g_time_pts);

    mk_frame frame = mk_frame_create(MKCodecH265, g_time_pts,g_time_pts,g_frame_buf, frame_len, NULL, NULL);
    if(NULL == frame)
    {
        sample_print("mk_frame_create error\n");
    }

    ret_a = mk_media_input_frame(media, frame);
    if (0 == ret_a ){
        sample_print("mk_media_input_frame error\n");
    }
    g_time_pts += g_time_pts_delta;
...
hi_void *sample_comm_venc_get_venc_stream_proc(hi_void *p)
{
    ...
    ret_a = mk_rtsp_server_start(554, 0);
    if (ret_a == 0)
    {
        sample_print("mk_rtsp_server_start failed!\n");
    }
    ...
    if(NULL == media)
    {
        sample_print("mk_media_create failed!\n");
        mk_stop_all_server();
        goto FREE;
    }
    codec_args v_args = {0};
    ...
    if(v_track == NULL)
    {
        sample_print("mk_track_create failed!\n");
        goto RTSP_ZLMEDIAKIT_FREE;
    }
