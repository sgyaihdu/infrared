...
static hi_s32 sample_svp_npu_init_media(hi_u32 is_file)
{
    hi_s32 ret;
    if (is_file) {
        ...
        ret = sample_common_svp_create_vb_start_vdec_vpss_vo(&g_svp_npu_vdec_cfg, &g_svp_npu_vdec_param,
            &g_svp_npu_vdec_thread, &g_svp_npu_media_cfg, &g_svp_npu_vo_cfg);
        sample_svp_check_exps_return(ret != HI_SUCCESS, ret, SAMPLE_SVP_ERR_LEVEL_DEBUG, "media init fail!\n");
    } else {
...
static hi_void sample_svp_npu_motr_acl_set_task_info_multimodel(hi_u32 model_idx_yolox, hi_u32 model_idx_motr,
    hi_u32 model_idx_qim)
{
    sample_svp_npu_acl_set_task_info(SAMPLE_YOLOX_TASK, model_idx_yolox);
    sample_svp_npu_acl_set_task_info(SAMPLE_MOTR_TASK, model_idx_motr);
    sample_svp_npu_acl_set_task_info(SAMPLE_QIM_TASK, model_idx_qim);
}

static hi_s32 sample_svp_npu_config_motr_resources(hi_void)
{
    hi_s32 ret;
    /* set motr params and get pic size */
    ret = sample_svp_npu_motr_set_params(SAMPLE_SVP_NPU_YOLOX_IDX, SAMPLE_SVP_NPU_MOTR_IDX);
    sample_svp_check_exps_return(ret != HI_SUCCESS, HI_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR, "get pic size fail!\n");

    /* set cfg */
    sample_svp_npu_motr_acl_set_task_info_multimodel(SAMPLE_SVP_NPU_YOLOX_IDX, SAMPLE_SVP_NPU_MOTR_IDX,
        SAMPLE_SVP_NPU_QIM_IDX);
    ret = sample_svp_npu_acl_init_task(SAMPLE_MOTR_TASK_NUM, HI_FALSE, 0); /* Three Models for three tasks */
    sample_svp_check_exps_return(ret != HI_SUCCESS, HI_FAILURE, SAMPLE_SVP_ERR_LEVEL_ERROR, "init task fail!\n");
...
hi_void sample_svp_npu_acl_e2e_hrnet(hi_void)
{
    hi_s32 ret;
    const hi_u32 model_idx = 0;
    const hi_char *om_model_path = "./data/model/hrnet.om";
    sample_svp_npu_thread_args args;
...
/* function : show the sample of motr [yolox->motr->qim] (vi -> vpss -> npu -> vgs -> vo) */
hi_void sample_svp_npu_acl_motr(hi_u32 is_file)
{
    hi_s32 ret;
    /* use PIC_BUTT to be a flag, get the input resolution form om */
    g_svp_npu_media_cfg.pic_type[SAMPLE_SVP_NPU_MOTR_FRAME] = PIC_BUTT;
    g_svp_npu_media_cfg.pic_type[SAMPLE_SVP_NPU_YOLOX_FRAME] = PIC_BUTT;

    /* set the initial terminate signal */
    g_svp_npu_terminate_signal = HI_FALSE;
    sample_svp_npu_thread_args args = {0}; /* Does not take effect in Motr Sample */

    if (g_svp_npu_terminate_signal == HI_FALSE) {
        /* init acl */
        ret = sample_svp_npu_acl_init(HI_NULL);
        sample_svp_check_exps_goto(ret != HI_SUCCESS, end0, SAMPLE_SVP_ERR_LEVEL_ERROR, "init failed!\n");

        /* load Motr models */
        ret = sample_common_svp_npu_load_model(SAMPLE_SVP_NPU_YOLOX_MODEL_PATH, SAMPLE_SVP_NPU_YOLOX_IDX, HI_FALSE);
        sample_svp_check_exps_goto(ret != HI_SUCCESS, end0, SAMPLE_SVP_ERR_LEVEL_ERROR, "load YOLOX fail!\n");
        ret = sample_common_svp_npu_load_model(SAMPLE_SVP_NPU_MOTR_MODEL_PATH, SAMPLE_SVP_NPU_MOTR_IDX, HI_FALSE);
        sample_svp_check_exps_goto(ret != HI_SUCCESS, end1_1, SAMPLE_SVP_ERR_LEVEL_ERROR, "load MOTR fail!\n");
        ret = sample_common_svp_npu_load_model(SAMPLE_SVP_NPU_QIM_MODEL_PATH, SAMPLE_SVP_NPU_QIM_IDX, HI_FALSE);
        sample_svp_check_exps_goto(ret != HI_SUCCESS, end1_2, SAMPLE_SVP_ERR_LEVEL_ERROR, "load QIM fail!\n");

        /* config resources: set specific config params + init npu task */
        ret = sample_svp_npu_config_motr_resources();
        sample_svp_check_exps_goto(ret != HI_SUCCESS, end1_3, SAMPLE_SVP_ERR_LEVEL_ERROR, "config resource fail\n");

        /* init media */
        sample_svp_npu_init_media(is_file);
        sample_svp_check_exps_goto(ret != HI_SUCCESS, end2, SAMPLE_SVP_ERR_LEVEL_DEBUG, "media init fail!\n");

        /* process */
        g_svp_npu_thread_stop = HI_FALSE;
        ret = pthread_create(&g_svp_npu_thread, 0, sample_svp_npu_motr_acl_vdec_to_vo, (hi_void*)&args);
        sample_svp_check_exps_goto(ret != HI_SUCCESS, end3, SAMPLE_SVP_ERR_LEVEL_ERROR, "create thread fail!\n");
        (hi_void)sample_svp_npu_acl_pause();
        g_svp_npu_thread_stop = HI_TRUE;
        pthread_join(g_svp_npu_thread, HI_NULL);
    }
end3:
    (hi_void)sample_svp_npu_acl_deinit_task(SAMPLE_MOTR_TASK_NUM, 0);
end2:
    if (is_file) {
        (hi_void)sample_common_svp_destroy_vb_stop_vdec_vpss_vo(&g_svp_npu_vdec_param, &g_svp_npu_vdec_thread,
            &g_svp_npu_media_cfg, &g_svp_npu_vo_cfg);
...
end1_3:
    (hi_void)sample_common_svp_npu_unload_model(SAMPLE_SVP_NPU_QIM_IDX);
end1_2:
    (hi_void)sample_common_svp_npu_unload_model(SAMPLE_SVP_NPU_MOTR_IDX);
end1_1:
    (hi_void)sample_common_svp_npu_unload_model(SAMPLE_SVP_NPU_YOLOX_IDX);
end0:
    (hi_void)sample_svp_npu_acl_deinit();
    (hi_void)sample_svp_npu_acl_terminate();
}