...
#define HI_SAMPLE_RESIZE_MAX_TILE_METRIC_8BIT   2032
#define HI_SAMPLE_RESIZE_MAX_TILE_METRIC_16BIT  1016
#define HI_SAMPLE_RESIZE_TILE_ADJUST            2
#define HI_SAMPLE_RESIZE_ASSIST_UNIT_SIZE       48
...
static hi_u32 sample_ive_resize_get_tile_metric(hi_u32 src_metric, hi_u32 dst_metric,
    hi_svp_img_type img_type)
{
    hi_s32 tmp_metric = HI_SAMPLE_RESIZE_MAX_TILE_METRIC_8BIT;
    hi_u32 src_tile_num, scale, dst_tmp_tile, src_tmp_tile, tile_remainder;
    switch (img_type) {
        ...
        case HI_SVP_IMG_TYPE_S8C1: {
            tmp_metric = HI_SAMPLE_RESIZE_MAX_TILE_METRIC_8BIT;
            break;
        }
        case HI_SVP_IMG_TYPE_U16C1:
        case HI_SVP_IMG_TYPE_S16C1: {
            tmp_metric = HI_SAMPLE_RESIZE_MAX_TILE_METRIC_16BIT;
            break;
        }
        default:
            break;
    ...
    if (dst_metric < src_metric) {
        ...
        if (tile_remainder == 0) {
            tmp_metric -= HI_SAMPLE_RESIZE_TILE_ADJUST;
        }
    } else if (dst_metric == src_metric || scale == 1) {
        tmp_metric -= HI_SAMPLE_RESIZE_TILE_ADJUST;
    }
    return (hi_u32)tmp_metric;
}
static hi_u32 sample_ive_resize_get_tile_num(hi_u32 width, hi_u32 height, hi_u32 tile_w, hi_u32 tile_h)
{
    hi_u32 x_num, y_num;
...
static hi_u32 sample_ive_resize_get_yuv420sp_tile_num(hi_u32 src_width, hi_u32 dst_width, hi_u32 src_height,
    hi_u32 dst_height)
{
    hi_u32 j, tile_width, tile_height;
    hi_u32 num = 0;
    tile_width = sample_ive_resize_get_tile_metric(src_width, dst_width, HI_SVP_IMG_TYPE_YUV420SP);
    for (j = 0; j < HI_SAMPLE_RESIZE_CBCR_NUM_2; j++) {
        tile_height = sample_ive_resize_get_tile_metric(src_height >> j, dst_height >> j, HI_SVP_IMG_TYPE_YUV420SP);
        num += sample_ive_resize_get_tile_num(dst_width, dst_height >> j, tile_width, tile_height);
    }
    return num;
}
 
static hi_u32 sample_ive_resize_get_yuv422sp_tile_num(hi_u32 src_width, hi_u32 dst_width, hi_u32 src_height,
    hi_u32 dst_height)
{
    hi_u32 j, tile_width, tile_height;
    hi_u32 num = 0;
    tile_width = sample_ive_resize_get_tile_metric(src_width, dst_width, HI_SVP_IMG_TYPE_YUV422SP);
    tile_height = sample_ive_resize_get_tile_metric(src_height, dst_height, HI_SVP_IMG_TYPE_YUV422SP);
    for (j = 0; j < HI_SAMPLE_RESIZE_CBCR_NUM_2; j++) {
        num += sample_ive_resize_get_tile_num(dst_width, dst_height, tile_width, tile_height);
    }
    return num;
}
 
static hi_u32 sample_ive_resize_get_u8c3_planar_tile_num(hi_u32 src_width, hi_u32 dst_width, hi_u32 src_height,
    hi_u32 dst_height)
{
    hi_u32 j, tile_width, tile_height;
    hi_u32 num = 0;
    tile_width = sample_ive_resize_get_tile_metric(src_width, dst_width, HI_SVP_IMG_TYPE_U8C3_PLANAR);
    tile_height = sample_ive_resize_get_tile_metric(src_height, dst_height, HI_SVP_IMG_TYPE_U8C3_PLANAR);
    for (j = 0; j < HI_SAMPLE_RESIZE_CBCR_NUM_3; j++) {
        num += sample_ive_resize_get_tile_num(dst_width, dst_height, tile_width, tile_height);
    }
    return num;
}
 
static hi_u32 sample_ive_resize_get_u8c1_tile_num(hi_u32 src_width, hi_u32 dst_width, hi_u32 src_height,
    hi_u32 dst_height)
{
    hi_u32 tile_width, tile_height;
    hi_u32 num = 0;
    tile_width = sample_ive_resize_get_tile_metric(src_width, dst_width, HI_SVP_IMG_TYPE_U8C1);
    tile_height = sample_ive_resize_get_tile_metric(src_height, dst_height, HI_SVP_IMG_TYPE_U8C1);
    num = sample_ive_resize_get_tile_num(dst_width, dst_height, tile_width, tile_height);
    return num;
}
 
static hi_u32 sample_ive_resize_get_u16c1_tile_num(hi_u32 src_width, hi_u32 dst_width, hi_u32 src_height,
    hi_u32 dst_height)
{
    hi_u32 tile_width, tile_height;
    hi_u32 num = 0;
    tile_width = sample_ive_resize_get_tile_metric(src_width, dst_width, HI_SVP_IMG_TYPE_U16C1);
    tile_height = sample_ive_resize_get_tile_metric(src_height, dst_height, HI_SVP_IMG_TYPE_U16C1);
    num = sample_ive_resize_get_tile_num(dst_width, dst_height, tile_width, tile_height);
    return num;
}
 
static hi_u32 sample_ive_resize_get_assist_buf_size(const hi_svp_src_img src[], const hi_svp_dst_img dst[],
    hi_u32 resize_num)
{
    hi_u32 i;
    hi_u32 num = 0;
    for (i = 0; i < resize_num; i++) {
        switch (src[i].type) {
            ...
            case HI_SVP_IMG_TYPE_YUV422SP: {
                num += sample_ive_resize_get_yuv422sp_tile_num(src[i].width, dst[i].width, src[i].height,
                    dst[i].height);
                break;
            }
            case HI_SVP_IMG_TYPE_U8C3_PLANAR: {
                num += sample_ive_resize_get_u8c3_planar_tile_num(src[i].width, dst[i].width, src[i].height,
                    dst[i].height);
                break;
            }
            case HI_SVP_IMG_TYPE_U8C1:
            case HI_SVP_IMG_TYPE_S8C1: {
                num += sample_ive_resize_get_u8c1_tile_num(src[i].width, dst[i].width, src[i].height,
                    dst[i].height);
                break;
            }
            case HI_SVP_IMG_TYPE_U16C1:
            case HI_SVP_IMG_TYPE_S16C1: {
                num += sample_ive_resize_get_u16c1_tile_num(src[i].width, dst[i].width, src[i].height,
                    dst[i].height);
                break;
            }
            default:
    ...
    return num * HI_SAMPLE_RESIZE_ASSIST_UNIT_SIZE;
}
/*
    ...
    hi_u32 src_height, hi_u32 dst_width, hi_u32 dst_height)
{
    ...
    resize_info->ctrl.mem.size = sample_ive_resize_get_assist_buf_size(resize_info->src, resize_info->dst,
        resize_info->ctrl.num);
