...
static hi_void *sample_ivs_md_proc(hi_void *args)
{
    ...
    hi_sample_md_vo_vpss_hld hld = {0};
    ...
    while (g_stop_signal == HI_FALSE) {
        ...
        ret = hi_mpi_vo_send_frame(hld.vo_layer, hld.vo_chn, &frm[0], HI_SAMPLE_IVE_MD_MILLIC_SEC);
        sample_svp_check_failed_err_level_goto(ret, base_free, "hi_mpi_vo_send_frame fail,Error(%#x)\n", ret);
