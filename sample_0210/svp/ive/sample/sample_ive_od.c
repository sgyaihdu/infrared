...
static hi_sample_svp_switch g_od_switch = { HI_FALSE, HI_TRUE };
...
static hi_s32 sample_ive_get_char_point(hi_sample_ive_od_info *od_ptr, hi_point char_point[], hi_u32 length)
{
    ...
    for (j = 0; (j < HI_IVE_CHAR_CALH) && (j < length); j++) {
        ...
        for (i = 0; i < HI_IVE_CHAR_CALW; i++) {
            ...
            /* mean */
            char_point[j * HI_IVE_CHAR_CALW + i].x = block_sum / (od_ptr->width * od_ptr->width);
            /* sigma=sqrt(1/(w*h)*sum((x(i,j)-mean)^2)= sqrt(sum(x(i,j)^2)/(w*h)-mean^2) */
            sqrt_val = block_sqrt / (od_ptr->width * od_ptr->height) - char_point[j * HI_IVE_CHAR_CALW + i].x *
                char_point[j * HI_IVE_CHAR_CALW + i].x;
            char_point[j * HI_IVE_CHAR_CALW + i].y = (hi_u32)sqrt(sqrt_val);
        }
    }
    return HI_SUCCESS;
}
...
static hi_void *sample_ive_od_proc(hi_void *arg)
{
    ...
    while (g_stop_signal == HI_FALSE) {
        ...
        /* send vo frame */
        ret = hi_mpi_vo_send_frame(0, 0, &base_frm_info, HI_SAMPLE_IVE_OD_MILLI_SEC);
