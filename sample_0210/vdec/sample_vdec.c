...
static hi_s32 sample_config_vpss_ldy_attr(hi_u32 vpss_grp_num)
{
    ...
    for (grp = 0; grp < (hi_s32)vpss_grp_num; grp++) {
        check_chn_return(hi_mpi_vpss_get_chn_low_delay(grp, 0, &vpss_ldy_info), grp, "hi_mpi_vpss_get_chn_low_delay");
        vpss_ldy_info.enable = HI_TRUE;
        vpss_ldy_info.line_cnt = 16; /* 16:VDEC_LOW_DELAY_MIN_LINE */
        check_chn_return(hi_mpi_vpss_set_chn_low_delay(grp, 0, &vpss_ldy_info), grp, "hi_mpi_vpss_set_chn_low_delay");
    }
...
static hi_s32 sample_start_vo(sample_vo_cfg *vo_config, hi_u32 vpss_grp_num)
{
    hi_s32 ret;
