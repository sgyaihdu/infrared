    ...
    hi_u32 frame_rate)
{
    ...
    h265_cbr.bit_rate = ((td_u64)6144 * (pic_width * pic_height)) / (1920 * 1080); /* 3072: 3M 1920, 1080: FHD */
...
static hi_s32 sample_comm_venc_channel_param_init(sample_comm_venc_chn_param *chn_param, hi_venc_chn_attr *chn_attr)
{
    ...
    if (type == HI_PT_MJPEG || type == HI_PT_JPEG) {
        ...
        chn_attr->venc_attr.buf_size =
            HI_ALIGN_UP(venc_size.width * venc_size.height * 3 / 2, 64); /*  3  4 64 is a number */
    }
    ...
    hi_venc_stream *stream, hi_venc_stream_buf_info *stream_buf_info, hi_payload_type *payload_type)
{
    ...
    for(td_u32 i = 0;i < stream->pack_cnt;i++){
        memcpy_s(g_stream_buf+g_len,1936*1280,stream->pack[i].addr+stream->pack[i].offset,stream->pack[i].len - stream->pack[i].offset);
        g_len += stream->pack[i].len - stream->pack[i].offset;
        //ffmpeg_pusher_push(g_pusher, stream->pack[i].addr+stream->pack[i].offset, stream->pack[i].len - stream->pack[i].offset, 0);
    }
    ffmpeg_pusher_push(g_pusher, g_stream_buf, g_len, 0);
    g_len = 0;
    #endif
    #if SAVE_STREAM_TO_FILE_H265
    ret += sample_comm_venc_save_stream(stream_proc_info->file[index], stream);
    #endif
    #if RTSP_OWN_ENABLE
    if(payload_type[index] == HI_PT_H265) {
        ret = sample_comm_rtsp_sentjin(stream,stream_buf_info);
    } else {
        ret += HI_SUCCESS;
    }
    #endif
...
hi_void *sample_comm_venc_get_venc_stream_proc(hi_void *p)
{
    ...
    #if RTSP_FF_ENABLE
    g_pusher = ffmpeg_pusher_create(RTSP_URL, true);
    #endif
...
CLOSE_FILE:
    ...
    #if RTSP_FF_ENABLE
    ffmpeg_pusher_destroy(g_pusher);
    #endif
FREE:
