...
#include "hw_dc_cmos_ex.h"
#include "hw_dc_cmos.h"

#define HW_DC_ID                    37
#define SENSOR_HW_DC_WIDTH          512
#define SENSOR_HW_DC_HEIGHT         768
 ...
 * global variables                                                            *
 ****************************************************************************/
#define hw_dc_sensor_set_ctx(pipe, ctx)   ((g_hw_dc_sns_state[pipe]) = (ctx))
#define hw_dc_sensor_reset_ctx(pipe)      (g_hw_dc_sns_state[pipe] = TD_NULL)
...
hw_dc_state g_hw_dc_state[OT_ISP_MAX_PIPE_NUM] = {{0}};
ot_isp_sns_commbus g_hw_dc_bus_info[OT_ISP_MAX_PIPE_NUM] = {
    [0] = { .i2c_dev = 0},
    [1 ... OT_ISP_MAX_PIPE_NUM - 1] = { .i2c_dev = -1}
};

ot_isp_sns_state *g_hw_dc_sns_state[OT_ISP_MAX_PIPE_NUM] = {TD_NULL};

static td_bool blc_clamp_info[OT_ISP_MAX_PIPE_NUM] = {[0 ...(OT_ISP_MAX_PIPE_NUM - 1)] = TD_TRUE};
ot_isp_sns_commbus *hw_dc_get_bus_info(ot_vi_pipe vi_pipe)
{
    return &g_hw_dc_bus_info[vi_pipe];
}

ot_isp_sns_state *hw_dc_get_ctx(ot_vi_pipe vi_pipe)
{
    return g_hw_dc_sns_state[vi_pipe];
}

td_void hw_dc_set_blc_clamp_value(ot_vi_pipe vi_pipe, td_bool clamp_en)
{
    blc_clamp_info[vi_pipe] = clamp_en;
}

const hw_dc_video_mode_tbl g_hw_dc_mode_tbl[HW_DC_MODE_BUTT] = {
    {HW_DC_VMAX_4K2K_LINEAR,   HW_DC_FULL_LINES_MAX,          30, 0.07,
     3840, 2160, 0, OT_WDR_MODE_NONE,       "HW_DC_SENSOR_8M_30FPS_LINEAR_MODE"},
};
...
#define  HW_DC_VMAX_ADDR_L                           0x3024
#define  HW_DC_VMAX_ADDR_M                           0x3025
#define  HW_DC_VMAX_ADDR_H                           0x3026
#define  HW_DC_HMAX_ADDR_L                           0x3028
#define  HW_DC_HMAX_ADDR_H                           0x3029

#define  HW_DC_SHR0_L                                0x3050
#define  HW_DC_SHR0_M                                0x3051
#define  HW_DC_SHR0_H                                0x3052
#define  HW_DC_GAIN_PGC_L                            0x3090
#define  HW_DC_GAIN_PGC_H                            0x3091

// sensor gain
#define HW_DC_AGAIN_MIN    1024
#define HW_DC_AGAIN_MAX    32381

#define HW_DC_DGAIN_MIN    1024
#define HW_DC_DGAIN_MAX    128914

#define HW_DC_AD_GAIN_TBL_RANGE  241
#define HW_DC_AGAIN_TBL_RANGE  100
#define HW_DC_DGAIN_TBL_RANGE  140

#define HW_DC_EXP_TIME_LMT    8


#define hw_dc_err_mode_print(sns_image_mode, sns_state) \
    do { \
    ...
    const ot_isp_sns_state *sns_state)
{
    ...
    ae_sns_dft->full_lines_max = HW_DC_FULL_LINES_MAX;
...
static td_void cmos_get_ae_linear_default(ot_vi_pipe vi_pipe, ot_isp_ae_sensor_default *ae_sns_dft,
    const ot_isp_sns_state *sns_state)
{
    ae_sns_dft->max_again = HW_DC_AGAIN_MAX;
    ae_sns_dft->min_again = HW_DC_AGAIN_MIN;
    ae_sns_dft->max_again_target = ae_sns_dft->max_again;
    ae_sns_dft->min_again_target = ae_sns_dft->min_again;

    ae_sns_dft->max_dgain = HW_DC_DGAIN_MAX;
    ae_sns_dft->min_dgain = HW_DC_DGAIN_MIN;
...
static td_s32 cmos_get_ae_default(ot_vi_pipe vi_pipe, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    hw_dc_sensor_get_ctx(vi_pipe, sns_state);
...
static td_void cmos_fps_set(ot_vi_pipe vi_pipe, td_float fps, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    lines = g_hw_dc_mode_tbl[sns_state->img_mode].ver_lines;
    lines_max = g_hw_dc_mode_tbl[sns_state->img_mode].max_ver_lines;
    max_fps = g_hw_dc_mode_tbl[sns_state->img_mode].max_fps;
    min_fps = g_hw_dc_mode_tbl[sns_state->img_mode].min_fps;
    ...
    ae_sns_dft->fps = fps;
    ae_sns_dft->full_lines_std = sns_state->fl_std;
    ae_sns_dft->max_int_time = sns_state->fl_std - HW_DC_EXP_TIME_LMT;
...
static td_void cmos_slow_framerate_set(ot_vi_pipe vi_pipe, td_u32 full_lines, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    ae_sns_dft->full_lines = sns_state->fl[0];
    ae_sns_dft->max_int_time = sns_state->fl[0] - HW_DC_EXP_TIME_LMT;
...
static td_void cmos_inttime_update_linear(ot_vi_pipe vi_pipe, td_u32 int_time)
{
    ...
    value = min(value, HW_DC_FULL_LINES_MAX);
...
static td_u32 g_gain_table[HW_DC_AD_GAIN_TBL_RANGE] = {
...
static td_void cmos_again_calc_table(ot_vi_pipe vi_pipe, td_u32 *again_lin, td_u32 *again_db)
{
    ...
    if (*again_lin >= g_gain_table[HW_DC_AGAIN_TBL_RANGE]) {
        *again_lin = g_gain_table[HW_DC_AGAIN_TBL_RANGE];
        *again_db = HW_DC_AGAIN_TBL_RANGE;
        return;
    } else {
        int i;
        for (i = 1; i < (HW_DC_AGAIN_TBL_RANGE + 1); i++) {
...
static td_void cmos_dgain_calc_table(ot_vi_pipe vi_pipe, td_u32 *dgain_lin, td_u32 *dgain_db)
{
    ...
    if (*dgain_lin >= g_gain_table[HW_DC_DGAIN_TBL_RANGE]) {
        *dgain_lin = g_gain_table[HW_DC_DGAIN_TBL_RANGE];
        *dgain_db = HW_DC_DGAIN_TBL_RANGE;
        return;
    }

    for (i = 1; i < (HW_DC_DGAIN_TBL_RANGE + 1); i++) {
...
static td_void cmos_get_isp_dng_default(const ot_isp_sns_state *sns_state, ot_isp_cmos_default *isp_def)
{
    ...
    switch (sns_state->img_mode) {
        case HW_DC_SENSOR_8M_30FPS_LINEAR_MODE:
...
static td_s32 cmos_get_isp_default(ot_vi_pipe vi_pipe, ot_isp_cmos_default *isp_def)
{
    ...
    isp_def->sns_mode.sns_id= HW_DC_ID;
...
static td_void cmos_set_pixel_detect(ot_vi_pipe vi_pipe, td_bool enable)
{
    ...
    if (sns_state->wdr_mode == OT_WDR_MODE_2To1_LINE) {
        ...
        if (sns_state->img_mode == HW_DC_SENSOR_8M_30FPS_LINEAR_MODE) {
            full_lines_5fps = (HW_DC_VMAX_4K2K_LINEAR * 30) / 5; /* 30fps, 5fps */
        } else {
            return;
        }
    }

    max_int_time_5fps = HW_DC_EXP_TIME_LMT;

    if (enable) { /* setup for ISP pixel calibration mode */
        hw_dc_write_register(vi_pipe, HW_DC_GAIN_PGC_L, 0x00);
        hw_dc_write_register(vi_pipe, HW_DC_GAIN_PGC_H, 0x00);

        hw_dc_write_register(vi_pipe, HW_DC_VMAX_ADDR_L, low_8bits(full_lines_5fps));
        hw_dc_write_register(vi_pipe, HW_DC_VMAX_ADDR_M, high_8bits(full_lines_5fps));
        hw_dc_write_register(vi_pipe, HW_DC_VMAX_ADDR_H, higher_4bits(full_lines_5fps));

        hw_dc_write_register(vi_pipe, HW_DC_SHR0_L, low_8bits(max_int_time_5fps));
        hw_dc_write_register(vi_pipe, HW_DC_SHR0_M, high_8bits(max_int_time_5fps));
        hw_dc_write_register(vi_pipe, HW_DC_SHR0_H, higher_4bits(max_int_time_5fps));
    } else { /* setup for ISP 'normal mode' */
        sns_state->fl_std = (sns_state->fl_std > HW_DC_FULL_LINES_MAX) ?
            HW_DC_FULL_LINES_MAX : sns_state->fl_std;
        hw_dc_write_register(vi_pipe, HW_DC_VMAX_ADDR_L, low_8bits(sns_state->fl_std));
        hw_dc_write_register(vi_pipe, HW_DC_VMAX_ADDR_M, high_8bits(sns_state->fl_std));
        hw_dc_write_register(vi_pipe, HW_DC_VMAX_ADDR_H, higher_4bits(sns_state->fl_std));
...
static td_void cmos_comm_sns_reg_info_init(ot_vi_pipe vi_pipe, ot_isp_sns_state *sns_state)
{
    ...
    sns_state->regs_info[0].com_bus.i2c_dev = g_hw_dc_bus_info[vi_pipe].i2c_dev;
    ...
    for (i = 0; i < sns_state->regs_info[0].reg_num; i++) {
        ...
        sns_state->regs_info[0].i2c_data[i].dev_addr = HW_DC_I2C_ADDR;
        sns_state->regs_info[0].i2c_data[i].addr_byte_num = HW_DC_ADDR_BYTE;
        sns_state->regs_info[0].i2c_data[i].data_byte_num = HW_DC_DATA_BYTE;
    }

    /* shutter related */
    sns_state->regs_info[0].i2c_data[0].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[0].reg_addr = HW_DC_SHR0_L;
    sns_state->regs_info[0].i2c_data[1].delay_frame_num = 0;
    sns_state->regs_info[0].i2c_data[1].reg_addr = HW_DC_SHR0_M;
    sns_state->regs_info[0].i2c_data[2].delay_frame_num = 0; /* index 2 */
    sns_state->regs_info[0].i2c_data[2].reg_addr = HW_DC_SHR0_H; /* index 2 */

    sns_state->regs_info[0].i2c_data[3].delay_frame_num = 0; /* index 3 */
    sns_state->regs_info[0].i2c_data[3].reg_addr = HW_DC_GAIN_PGC_L; /* index 3 */
    sns_state->regs_info[0].i2c_data[4].delay_frame_num = 0;  /* index 4 */
    sns_state->regs_info[0].i2c_data[4].reg_addr = HW_DC_GAIN_PGC_H; /* index 4 */
    /* Vmax */
    sns_state->regs_info[0].i2c_data[5].delay_frame_num = 0; /* index 5 */
    sns_state->regs_info[0].i2c_data[5].reg_addr = HW_DC_VMAX_ADDR_L; /* index 5 */
    sns_state->regs_info[0].i2c_data[6].delay_frame_num = 0; /* index 6 */
    sns_state->regs_info[0].i2c_data[6].reg_addr = HW_DC_VMAX_ADDR_M; /* index 6 */
    sns_state->regs_info[0].i2c_data[7].delay_frame_num = 0; /* index 7 */
    sns_state->regs_info[0].i2c_data[7].reg_addr = HW_DC_VMAX_ADDR_H; /* index 7, index 2 */
    return;
}
    ...
    ot_isp_sns_state *sns_state)
{
    switch (sns_image_mode) {
        case HW_DC_SENSOR_8M_30FPS_LINEAR_MODE:
            sns_state->fl_std         = HW_DC_VMAX_4K2K_LINEAR;
            break;
        default:
            break;
    }
...
static td_s32 cmos_set_image_mode(ot_vi_pipe vi_pipe, const ot_isp_cmos_sns_image_mode *sns_image_mode)
{
    ...
    for (i = 0; i < HW_DC_MODE_BUTT; i++) {
        if (sns_image_mode->fps <= g_hw_dc_mode_tbl[i].max_fps &&
            sns_image_mode->width <= g_hw_dc_mode_tbl[i].width &&
            sns_image_mode->height <= g_hw_dc_mode_tbl[i].height &&
            sns_state->wdr_mode == g_hw_dc_mode_tbl[i].wdr_mode) {
            image_mode = (hw_dc_res_mode)i;
            break;
        }
    }

    if (i >= HW_DC_MODE_BUTT) {
        hw_dc_err_mode_print(sns_image_mode, sns_state);
        return TD_FAILURE;
    }
...
static td_void sensor_global_init(ot_vi_pipe vi_pipe)
{
    ...
    sns_state->sync_init = TD_FALSE;
    sns_state->img_mode = HW_DC_SENSOR_8M_30FPS_LINEAR_MODE;
    sns_state->wdr_mode = OT_WDR_MODE_NONE;
    sns_state->fl_std  = HW_DC_VMAX_4K2K_LINEAR;
    sns_state->fl[0] = HW_DC_VMAX_4K2K_LINEAR;
    sns_state->fl[1] = HW_DC_VMAX_4K2K_LINEAR;
...
static td_s32 cmos_init_sensor_exp_function(ot_isp_sns_exp_func *sensor_exp_func)
{
    ...
    sensor_exp_func->pfn_cmos_sns_init         = hw_dc_init;
    sensor_exp_func->pfn_cmos_sns_exit         = hw_dc_exit;
...
static td_s32 hw_dc_set_bus_info(ot_vi_pipe vi_pipe, ot_isp_sns_commbus sns_bus_info)
{
    return TD_SUCCESS;   
    g_hw_dc_bus_info[vi_pipe].i2c_dev = sns_bus_info.i2c_dev;
...
static td_s32 sensor_ctx_init(ot_vi_pipe vi_pipe)
{
    ...
    hw_dc_sensor_get_ctx(vi_pipe, sns_state_ctx);
    ...
    hw_dc_sensor_set_ctx(vi_pipe, sns_state_ctx);
...
static td_void sensor_ctx_exit(ot_vi_pipe vi_pipe)
{
    ...
    hw_dc_sensor_reset_ctx(vi_pipe);
    return;
}
...
static td_s32 sensor_unregister_callback(ot_vi_pipe vi_pipe, ot_isp_3a_alg_lib *ae_lib, ot_isp_3a_alg_lib *awb_lib)
{
    ...
    ret = ot_mpi_isp_sensor_unreg_callback(vi_pipe, HW_DC_ID);
    if (ret != TD_SUCCESS) {
        isp_err_trace("sensor unregister callback function failed!\n");
        return ret;
    }

    ret = ot_mpi_ae_sensor_unreg_callback(vi_pipe, ae_lib, HW_DC_ID);
    if (ret != TD_SUCCESS) {
        isp_err_trace("sensor unregister callback function to ae lib failed!\n");
        return ret;
    }

    ret = ot_mpi_awb_sensor_unreg_callback(vi_pipe, awb_lib, HW_DC_ID);
    if (ret != TD_SUCCESS) {
...
ot_isp_sns_obj g_sns_hw_dc_obj = {
    .pfn_register_callback     = sensor_register_callback,
    .pfn_un_register_callback  = sensor_unregister_callback,
    .pfn_standby               = hw_dc_standby,
    .pfn_restart               = hw_dc_restart,
    .pfn_mirror_flip           = TD_NULL,
    .pfn_set_blc_clamp         = hw_dc_blc_clamp,
    .pfn_write_reg             = hw_dc_write_register,
    .pfn_read_reg              = hw_dc_read_register,
    .pfn_set_bus_info          = hw_dc_set_bus_info,
