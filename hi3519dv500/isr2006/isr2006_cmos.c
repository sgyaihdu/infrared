...
const isr2006_video_mode_tbl g_isr2006_mode_tbl[ISR2006_MODE_BUTT] = {
    {ISR2006_VMAX_30FPS_LINEAR, ISR2006_FULL_LINES_MAX, 25, 5,
                                          ...
                                          const ot_isp_sns_state *sns_state)
{
    ...
    ae_sns_dft->max_int_time = ISR2006_VMAX_25FPS_LINEAR - 2; /* sub 2 */
...
static td_void cmos_fps_set(ot_vi_pipe vi_pipe, td_float fps, ot_isp_ae_sensor_default *ae_sns_dft)
{
    ...
    lines = g_isr2006_mode_tbl[0].ver_lines;
...
static td_void cmos_inttime_update_linear(ot_vi_pipe vi_pipe, td_u32 int_time)
{
    ...
    value = min(max(value, ISR2006_EXP_TIME_LMT), (ISR2006_VMAX_25FPS_LINEAR - 1));
    ...
    // sns_state->regs_info[0].i2c_data[11].data = low_8bits(ISR2006_VMAX_25FPS_LINEAR - 20 - value - 1);
    // sns_state->regs_info[0].i2c_data[12].data = high_8bits(ISR2006_VMAX_25FPS_LINEAR - 20 - value - 1);
    // sns_state->regs_info[0].i2c_data[13].data = higher_8bits(ISR2006_VMAX_25FPS_LINEAR - 20 -value - 1);
    ...
    // sns_state->fl_std = ISR2006_VMAX_4K2K_LINEAR > 32 + value +1 ? ISR2006_VMAX_4K2K_LINEAR : 32 + value + 1;
    return;
}
...
static td_s32 cmos_set_image_mode(ot_vi_pipe vi_pipe, const ot_isp_cmos_sns_image_mode *sns_image_mode)
{
    ...
    if (sns_image_mode->fps == 30)
    {
        if ((sns_image_mode->width == 1936) && sns_image_mode->height == 1280)
        {
            image_mode = ISR2006_SENSOR_1936_1280_30FPS_12BIT_LINEAR_MODE;
        }
        else if (sns_image_mode->width == 1920 && sns_image_mode->height == 1080)
        {
            image_mode = ISR2006_SENSOR_1920_1080_30FPS_14BIT_LINEAR_MODE;
        }
        else
        {
            image_mode = ISR2006_MODE_BUTT;
        }
    }
    else if (sns_image_mode->fps == 25)
    {
        if (((sns_image_mode->width == 1936) || (sns_image_mode->width == 968)) && (sns_image_mode->height == 1280 || (sns_image_mode->height == 640)))
        {
            image_mode = ISR2006_SENSOR_1936_1280_25FPS_12BIT_LINEAR_MODE;
            if(sns_image_mode->width == 968)
            {
                image_mode = ISR2006_SENSOR_968_640_25FPS_12BIT_LINEAR_MODE;
            }
