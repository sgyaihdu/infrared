...
static unsigned int parse_sensor_clock(const char *name, unsigned int name_len)
{
    ...
    if ((strncmp(SENSOR_NAME_OS08A20, name, len) == 0) ||
        ...
        clock = 0xA001;//27MHZ
    } else if ((strncmp(SENSOR_NAME_DC,name,len)) == 0) {
        clock = 0x2001;//54MHZ
        printk("dc sensor clock is %#x\n", clock);
    } else {
        return clock;
    }
...
static int is_coms_bt601(const char *name, unsigned int name_len)
{
    unsigned int len;
...
static int is_coms_dc(const char *name, unsigned int name_len)
{
    unsigned int len;
...
static int coms_clock_config(int index)
{
    if (index == 1) { /* port1 */
        ...
        sys_writel(g_reg_crg_base,  0x8440, 0x3); /* sensor0 */
        sys_writel(g_reg_crg_base,  0x8440, 0xa010);
...
static void prase_sensor(char *s)
{
    ...
    if (ret >= 0) {
        ...
        else if ((index == 1 || index == 3) && is_coms_bt601(sensor_name, SENSOR_NAME_LEN)) {
            g_vi_intf_type = VI_CMOS_BT601_MODE;} 
        else if ((index == 1 || index == 3) && is_coms_dc(sensor_name, SENSOR_NAME_LEN)) {
            g_vi_intf_type = VI_CMOS_DC_MODE;} 
...
static void sensor_cfg(void)
{
    ...
    for (i = 0; i < SENSOR_NUM; i++) {
        ...
        if (is_coms_bt1120(g_vi_sensor_name[i], SENSOR_NAME_LEN) ||
            ...
            if (i != 0 || sensor2_clk_use_ad == 0) { /* ad use sensor0 clk */
